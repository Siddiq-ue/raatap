-- =================================================================
-- FIX: overlap distance measured against a straight chord instead of
-- the host's real, road-following route
-- =================================================================
-- Bug: calculate_overlapping_distance() (from migration 19) checked
-- whether the rider's pickup/dropoff were "on route" by measuring
-- distance to ST_MakeLine(host_from, host_to) - a straight line
-- between the host's two endpoints - with a hardcoded 500m cutoff.
--
-- Real routes bend. A rider can be a genuine ~48m detour away by road
-- (OSRM) while sitting ~500m+ from the idealized straight chord, so
-- the on-route check silently failed and overlapping_distance_meters
-- (and therefore the rider's estimated cost) came back 0 even though
-- the rider's trip sits almost entirely inside the host's route.
--
-- Fix: when the host's actual route_geometry (GEOGRAPHY(LineString,
-- 4326), already stored on ride_templates) is available, project the
-- rider's pickup/dropoff onto that real polyline instead of the
-- chord. Falls back to the chord only if no geometry is stored.
-- =================================================================
...
-- ----------------------------------------------------------------
-- calculate_route_match_score: pass the host's real route geometry
-- through to calculate_overlapping_distance instead of only the
-- endpoints.
-- ----------------------------------------------------------------
...
BEGIN
    ...
    -- REAL route geometry when available (falls back to the chord otherwise).
    -- This, NOT the host's full route distance, is what the rider should be
    -- charged for.
...
calculate_overlapping_distance() (now projected onto the host''s real route
geometry when available) to determine the rider''s billable segment.';

-- Note: backfilling overlapping_distance_meters for existing pending
-- match_suggestions is done separately (scripts/backfill-overlap-via-rpc.js)
-- rather than in this migration - each row's calculate_overlapping_distance()
-- call makes a live OSRM HTTP request, which is too slow to run reliably
-- inside the migration runner's transaction/timeout.
